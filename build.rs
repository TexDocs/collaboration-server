...
fn main() {
    ...
    let mut hash = if cfg!(any(unix)) {
        ...
        panic!("You shall not pas...ehm..compile on a non-unix OS!")
    };
    hash.pop();
    ...
    f.write_all(
        "//! A dynamically generated file containing the current hash of the repository\n"
            .to_string()
            .as_bytes(),
    ).unwrap();
    f.write_all(
        "/// The current hash of the project\n"
            .to_string()
            .as_bytes(),
    ).unwrap();
    f.write_all(
